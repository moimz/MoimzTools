...
class MoimzToolsInstaller {
    ...
    async init() {
        ...
        const language = document.querySelector('input[name=language]') as HTMLInputElement;
    ...
    initEvent(): void {
        document.querySelectorAll('div[data-role=select]').forEach((dom) => {
            dom.querySelector('button').addEventListener('click', (e: MouseEvent) => {
                const target = e.currentTarget as Element;
                (target.parentNode as Element).classList.add('extend');
            ...
            dom.querySelectorAll('li[data-value]').forEach((dom) => {
                dom.addEventListener('click', (e: MouseEvent) => {
                    ...
                    const input = document.querySelector(
                        'input[name=' + parent.getAttribute('data-name') + ']'
                    ) as HTMLInputElement;
        ...
        document.querySelectorAll('input').forEach((input: HTMLInputElement) => {
            ...
            if (this.getValue(name) !== null) {
                if (type == 'checkbox') {
                    input.checked = this.getValue(name) as boolean;
                } else {
                    input.value = this.getValue(name) as string;
                }
            }
    ...
    async getPackage(): Promise<Package> {
        if (this.package !== null) {
            ...
            this.package = (await this.getApi('../package.json')) as Package;
            return this.package;
        }
    }
     ...
     */
    async getApi(url: string, params: Object = {}, is_retry: boolean | number = true): Promise<Results> {
     ...
     */
    async getLanguages(): Promise<{ [key: string]: any }> {
     ...
     */
    async getHtml(name: string, version: string, is_retry: boolean | number = true): Promise<string> {
        let retry = (is_retry === false ? 10 : is_retry) as number;

        try {
            const response: Response = (await fetch(
                '//www.moimz.tools/installer/index.php?name=' + name + '&version=' + version,
                {
                    method: 'GET',
                    headers: {
                        'Accept-Language': this.getLanguage(),
     ...
     */
    async getText(code: string): Promise<string> {
        if (this.text === null) {
            this.text = (await this.getApi(
                '//www.moimz.tools/installer/languages/' + this.getLanguage() + '.json'
            )) as Language;
        }
        ...
        for (const text of texts) {
            if (replacements !== null && replacements[text[1]] !== undefined) {
                origin = origin.replace(
                    text[0],
                    is_bold == true ? '<b>' + replacements[text[1]] + '</b>' : replacements[text[1]]
                );
            } else {
                origin = origin.replace(
                    text[0],
                    is_bold == true ? '<b>' + (await this.getText(text[1])) + '</b>' : await this.getText(text[1])
                );
            }
        }
     ...
     */
    getValue(name: string): boolean | string {
    ...
    setValue(name: string): void {
        if (name.indexOf('password') > -1 || name == 'key') return;
     ...
     */
    activeStep(step: string, is_visible: boolean = true): void {
        const steps = this.getSteps();
        document.querySelector('main').setAttribute('data-step', step);
        document.querySelectorAll('div[data-step]').forEach((dom: HTMLElement) => {
            dom.style.display = 'none';
        });

        if (is_visible == true) {
            (document.querySelector('div[data-step=' + step + ']') as HTMLElement).style.display = 'block';
            history.replaceState(
                null,
                document.title,
                location.pathname + '?language=' + this.getLanguage() + '&step=' + step
            );
        }
    ...
    checkStep(): boolean {
        ...
        const current = document.querySelector('div[data-step=' + step + ']') as HTMLElement;

        let checked = true;

        switch (step) {
            case 'configs':
                const token = current.querySelector('input[name=token]') as HTMLInputElement;
                if (current.style.display == 'block' || token.value.length == 0) {
                    for (const input of current.querySelectorAll('input')) {
                        if (input.getAttribute('disabled') == 'disabled') continue;
                        if (input.getAttribute('name') == 'token') continue;
     ...
     * @return {boolean} is_move - 이동여부
     */
    nextStep(is_visible: boolean = true): boolean {
        const step = this.getStep();
        ...
        if (step == 'configs') {
            if (is_visible == true) {
                ...
                const token = document.querySelector('div[data-step=configs] input[name=token]') as HTMLInputElement;
     ...
     */
    async getCheckerTag(type: string, key: string, results: { [key: string]: any } = null): Promise<HTMLElement> {
     ...
     */
    async checkRequirements(is_refresh: boolean = false): Promise<void> {
        const success = document.querySelector('input[name=requirements_success]') as HTMLInputElement;
        const checked = document.querySelector('input[name=requirements_checked]') as HTMLInputElement;
        ...
        document.querySelectorAll('div[data-step=requirements] li[data-type]').forEach(async (dom) => {
            dom.replaceWith(
                await this.getCheckerTag(
                    dom.getAttribute('data-type'),
                    dom.getAttribute('data-key'),
                    requirements[dom.getAttribute('data-key')] ?? null
                )
            );
        });
     ...
     * @return {Promise<void>} promise
     */
    async checkConfigs(): Promise<void> {
        const loading = document.querySelector('div[data-step=configs] div[data-role=loading]') as HTMLElement;
        ...
        if (configs?.has_configs === true) {
            ...
            document.querySelectorAll('div[data-step=configs] input').forEach((input: HTMLInputElement) => {
                if (
                    input.getAttribute('name') == 'token' ||
     ...
     */
    async submitConfigs(): Promise<boolean> {
        let configs = {};
        document.querySelectorAll('div[data-step=configs] input').forEach((input: HTMLInputElement) => {
            configs[input.getAttribute('name')] = input.value.length > 0 ? input.value : null;
        });

        document.querySelectorAll('div[data-step=configs] div[data-role=input]').forEach(async (dom: HTMLElement) => {
        ...
        const success = results.success as boolean;
        ...
        if (success == true) {
            ...
            if (results.code) {
                const code = document.querySelector('div[data-step=configs] div[data-role=code]') as HTMLElement;
                ...
                for (const name in results.errors) {
                    const input = document.querySelector(
                        'div[data-step=configs] input[name=' + name + ']'
                    ) as HTMLInputElement;
                    const help = document.querySelector(
                        'div[data-step=configs] div[data-role=help][data-name=' + name + ']'
                    );

                    if (input !== null) {
                        const value = results.errors[name] as string;
                    ...
                    if (help !== null) {
                        if (input === null) {
                            const value = results.errors[name] as {
                                text: string;
    ...
    async install(): Promise<boolean> {
        ...
        const installed = step.querySelector('input[name=installed]') as HTMLInputElement;
        installed.value = '';
        installed.dispatchEvent(new Event('input'));

        step.querySelectorAll('ul[data-role=core] > li[data-type]').forEach(async (checker: HTMLElement) => {
        ...
        step.querySelectorAll('ul[data-role=dependencies] > li[data-type]').forEach(async (checker: HTMLElement) => {
        ...
        const configs = await (
            await fetch('./process/index.php?action=install&step=configs&token=' + token, {
                method: 'POST',
                cache: 'no-store',
                redirect: 'follow',
            })
        ...
        if (configs !== null) {
            let checker = step.querySelector('li[data-type=configs]') as HTMLElement;
            let icon = checker.querySelector('i');
            icon.classList.remove(...icon.classList);
        ...
        const databases = await (
            await fetch('./process/index.php?action=install&step=databases&token=' + token, {
                method: 'POST',
                cache: 'no-store',
                redirect: 'follow',
            })
        ...
        success = success && configs?.success;
        if (databases !== null) {
            let checker = step.querySelector('li[data-type=databases]') as HTMLElement;
            let icon = checker.querySelector('i');
            icon.classList.remove(...icon.classList);
        ...
        if (success == true) {
            step.querySelectorAll('ul[data-role=dependencies] > li[data-type]').forEach(
                async (checker: HTMLElement) => {
                    const key = checker.getAttribute('data-key');

                    const dependencies = await (
                        await fetch(
                            './process/index.php?action=install&step=dependencies&key=' + key + '&token=' + token,
                            {
                                method: 'POST',
                                cache: 'no-store',
                                redirect: 'follow',
                    ...
                    if (dependencies?.success == true) {
                        checker.replaceWith(
                            await this.getCheckerTag('dependencies', key, {
                                status: 'success',
